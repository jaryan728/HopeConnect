module.exports = {
  ...
  async down(queryInterface, Sequelize) {
    ...
    try {
      ...
      await queryInterface.removeColumn('volunteers', 'availability', {
        transaction,
      });

      await queryInterface.removeColumn('volunteers', 'skills', {
        transaction,
      });

      await queryInterface.removeColumn('volunteers', 'userId', {
        transaction,
      });

      await queryInterface.removeColumn('reports', 'userId', { transaction });

      await queryInterface.removeColumn('reports', 'status', { transaction });

      await queryInterface.removeColumn('reports', 'location', { transaction });

      await queryInterface.removeColumn('reports', 'description', {
        transaction,
      });

      await queryInterface.removeColumn('donations', 'location', {
        transaction,
      });
